...
//
// Two input/output contracts are supported, selected by the "variant"
// option passed to buildNnEntryJs. Both are documented here for anyone
// training a model elsewhere and exporting to ONNX.
//
// variant "flat768" (original v1 contract, unchanged):
//   input  <session's first input>  float32 [1, 768]  -- 12 planes
//                                          (P N B R Q K p n b r q k)
//                                          x 64 squares, board() order
//                                          (rank 8 -> rank 1, file a -> h),
//                                          1.0 if that piece/color occupies
//                                          the square, else 0.0.
//   output <session's first output> float32 [1, 1]    -- position value FROM
//                                          THE PERSPECTIVE OF THE SIDE TO
//                                          MOVE, roughly in pawn units (the
//                                          driver multiplies by 100 to
//                                          report UCI centipawns).
//   Input/output tensor names are read from the ONNX session at runtime
//   (session.inputNames[0] / session.outputNames[0]), so any export tool's
//   naming works, not just literally "input"/"output".
//
// variant "lc0" (Leela Chess Zero / Maia compatible, classical format):
//   input  <session's first input>  float32 [1, 112, 8, 8] -- lc0's
//                                          INPUT_CLASSICAL_112_PLANE
//                                          encoding: 8 history steps x 13
//                                          planes (6 "our" piece planes, 6
//                                          "their" piece planes, 1
//                                          repetition plane), then 8
//                                          auxiliary planes (our/their
//                                          castling x2, side-to-move color,
//                                          rule50 count, an all-zero
//                                          deprecated plane, an all-ones
//                                          board-edge plane). The whole
//                                          tensor is expressed from the
//                                          perspective of the side to move
//                                          (board mirrored vertically, i.e.
//                                          rank flipped, when black is to
//                                          move -- files are never mirrored,
//                                          since this build never applies
//                                          lc0's canonicalization
//                                          transform, only its classical
//                                          format).
//   outputs are auto-detected by shape after a run: whichever output has
//                                          1858 elements is treated as the
//                                          policy, and whichever has 1
//                                          (scalar value) or 3 (WDL
//                                          win/draw/loss, from the side to
//                                          move's perspective) elements is
//                                          treated as the value.
//   The policy index for a legal move is looked up in lc0's own 1858-entry
//   move table (ported verbatim from lc0's src/neural/encoder.cc, see
//   LC0_MOVE_INDEX below), after flipping the move's squares the same way
//   the board is flipped. Knight underpromotions have no policy index in
//   lc0's encoding (a real limitation of the format itself, not of this
//   build) and are only ever played as a last-resort fallback.
//   This variant runs a single inference per "go" and picks the highest
//   scoring legal move directly from the policy head -- it does not run
//   the negamax search used by the flat768 variant. This matches how Maia
//   in particular is meant to be used (it's trained to imitate human move
//   choice, not to be searched), and avoids the much harder problem of
//   re-deriving the 8-ply history tensor for hypothetical positions deep
//   inside a search tree. If you want multi-ply search over an lc0-style
//   value head, say so and I'll build it, but it's a materially bigger
//   change (the history-per-node bookkeeping is the hard part, not the
//   search itself).
//
...
// the .chsengine.zip file itself is fully self-contained otherwise. No
// additional asset files are required for the "lc0" variant either: the
// move index table is embedded directly in entry.js below.
...
// Returns the lc0 policy index for a legal move (chess.js verbose move
// object), or -1 if the move has no representable index (this only ever
// happens for knight underpromotions, which lc0's move encoding cannot
// express -- see the format note at the top of this file).
function lc0MoveIndex(move, flip) {
  var from = flip ? flipSquare(move.from) : move.from;
  var to = flip ? flipSquare(move.to) : move.to;
  var key = from + to;
  // Queen promotion (and non-promotion moves) share the plain "fromto" key;
  // rook/bishop underpromotions get an explicit suffix; knight has none.
  if (move.promotion && move.promotion !== 'q') {
    if (move.promotion === 'n') return -1;
    key += move.promotion;
  }
  var idx = LC0_MOVE_TO_INDEX[key];
  return idx === undefined ? -1 : idx;
}

function encodePositionFlat(c) {
...
// Builds lc0's INPUT_CLASSICAL_112_PLANE tensor (112 x 8 x 8, flattened
// channel-major) from the tracked game history (see snapshotCurrent /
// positionHistoryBoards below). sideToMove is 'w' or 'b' for the CURRENT
// position; the whole tensor (all 8 history steps) is flipped vertically
// together if sideToMove is 'b' -- lc0 always presents the board as if the
// side to move is playing up the board from the bottom.
function buildLc0Planes(historyBoards, sideToMove, chessObj) {
  ...
  var n = historyBoards.length;
    ...
    for (var row = 0; row < 8; row++) {
      for (var col = 0; col < 8; col++) {
        ...
        if (!piece) continue;
        var rankIdx = 7 - row; // 0 = rank 1
        var r2 = flip ? 7 - rankIdx : rankIdx;
        var pIdx = PLANES6.indexOf(piece.type);
        var planeOffset;
        if (piece.color === oursColor) planeOffset = base + pIdx;
        else planeOffset = base + 6 + pIdx;
        data[planeOffset * 64 + r2 * 8 + col] = 1.0;
      }
    }
    if (snap.repetitions >= 1) {
      var repBase = (base + 12) * 64;
      for (var s = 0; s < 64; s++) data[repBase + s] = 1.0;
    }
  }
  var aux = 104;
  function fillPlane(idx) {
    var b = idx * 64;
    for (var s = 0; s < 64; s++) data[b + s] = 1.0;
  }
  var fenParts = chessObj.fen().split(' ');
  var castling = fenParts[2] || '-';
  var weWhite = sideToMove === 'w';
  var weCan000 = castling.indexOf(weWhite ? 'Q' : 'q') !== -1;
  var weCan00 = castling.indexOf(weWhite ? 'K' : 'k') !== -1;
  var theyCan000 = castling.indexOf(weWhite ? 'q' : 'Q') !== -1;
  var theyCan00 = castling.indexOf(weWhite ? 'k' : 'K') !== -1;
  if (weCan000) fillPlane(aux + 0);
  if (weCan00) fillPlane(aux + 1);
  if (theyCan000) fillPlane(aux + 2);
  if (theyCan00) fillPlane(aux + 3);
  if (sideToMove === 'b') fillPlane(aux + 4);
  var rule50 = parseInt(fenParts[4], 10) || 0;
  var rb = (aux + 5) * 64;
  for (var s2 = 0; s2 < 64; s2++) data[rb + s2] = rule50;
  // aux + 6 (deprecated movecount plane) stays all zero.
  fillPlane(aux + 7);
  return data;
}
...
async function evaluateAsync(c) {
  ...
  var feeds = {};
  feeds[inputName] = new ort.Tensor('float32', encodePositionFlat(c), [1, 768]);
  var results = await session.run(feeds);
  var value = results[outputName].data[0];
  return value * 100; // pawn units -> centipawns
}

var chess = new Chess();
var searchState = { stop: false };
var positionHistoryBoards = [];
...
async function searchBestMoveLc0(startTime) {
  var legalMoves = chess.moves({ verbose: true });
  if (legalMoves.length === 0) return { move: null, score: 0 };
  var sideToMove = chess.turn();
  var flip = sideToMove === 'b';
  var planes = buildLc0Planes(positionHistoryBoards, sideToMove, chess);
  var inputName = session.inputNames[0];
  var feeds = {};
  feeds[inputName] = new ort.Tensor('float32', planes, [1, 112, 8, 8]);
  var results = await session.run(feeds);
  var policyData = null;
  var valueData = null;
  for (var i = 0; i < session.outputNames.length; i++) {
    var out = results[session.outputNames[i]];
    if (!out || !out.data) continue;
    if (out.data.length === 1858) policyData = out.data;
    else if (out.data.length === 1 || out.data.length === 3) valueData = out.data;
  }
  var bestMove = null;
  var bestLogit = -Infinity;
  for (var m = 0; m < legalMoves.length; m++) {
    var idx = lc0MoveIndex(legalMoves[m], flip);
    if (idx === -1) continue; // unrepresentable knight underpromotion
    var logit = policyData ? policyData[idx] : 0;
    if (logit > bestLogit) {
      bestLogit = logit;
      bestMove = legalMoves[m];
    }
  }
  if (!bestMove) bestMove = legalMoves[0]; // e.g. only legal move is a knight underpromotion
  var cp = 0;
  if (valueData) {
    cp = valueData.length === 3
      ? Math.round((valueData[0] - valueData[2]) * 100)
      : Math.round(valueData[0] * 100);
  }
  var elapsed = Math.max(1, Date.now() - startTime);
  send(
    'info depth 1 score cp ' + cp +
    ' nodes 1 nps ' + Math.round(1000 / elapsed) +
    ' time ' + elapsed +
    ' pv ' + toUciMove(bestMove)
  );
  return { move: bestMove, score: cp };
}

function snapshotCurrent() {
  var f = chess.fen().split(' ');
  var key = f[0] + ' ' + f[1] + ' ' + f[2] + ' ' + f[3];
  snapshotCurrent._counts[key] = (snapshotCurrent._counts[key] || 0) + 1;
  positionHistoryBoards.push({ board: chess.board(), repetitions: snapshotCurrent._counts[key] - 1 });
}

function handlePosition(parts) {
  var idx = 1;
  positionHistoryBoards = [];
  snapshotCurrent._counts = {};
...
async function handleGo(parts) {
  ...
  if (VARIANT === 'lc0') {
    // Single policy-head inference, no negamax: see the format note at the
    // top of this file for why.
    var lc0Result = await searchBestMoveLc0(startTime);
    send('bestmove ' + (lc0Result.move ? toUciMove(lc0Result.move) : '(none)'));
    return;
  }
...
export function buildNnEntryJs({ name, author, defaultDepth = 1, variant = "flat768" }) {
  if (variant !== "flat768" && variant !== "lc0") {
    throw new Error('buildNnEntryJs: variant must be "flat768" or "lc0", got ' + JSON.stringify(variant));
  }
  return TEMPLATE.replace('/*__ENGINE_NAME__*/ "engine"', JSON.stringify(name || "engine"))
    ...
    .replace('/*__VARIANT__*/ "flat768"', JSON.stringify(variant));
}
